...
const PaymentMethodSchema = z.object({
  type: z.enum(['credit_card', 'debit_card', 'gcash', 'grabpay', 'maya', 'bank_transfer']),
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({ 
        ...
        error: providerError.response?.data?.errors?.[0]?.detail || providerError.message 
      });
    }
  ...
  try {
    ...
    if (event.data?.attributes?.type === 'payment_intent.payment_succeeded') {
      ...
      if (metadata?.user_id && metadata?.subscription) {
        ...
        console.log(`Subscription activated for user ${metadata.user_id}: ${metadata.subscription}`);
      }
    }
